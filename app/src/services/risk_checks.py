...
All functions are pure — no I/O, no AWS calls, no DB access.
compute_risk_flags() is the single entry point used by the summary endpoint.

Design principle: these functions determine facts (ownership match, naming
compliance, environment risk). Bedrock is never consulted to determine facts;
it only receives the pre-computed output of these functions to narrate as prose.
"""
...
def compute_risk_flags(
    ...
    """
    ownership_mismatch = check_ownership_mismatch(requested_by, service.owner_email)
    naming_violations = check_naming_convention(
        secret_request.logical_name, secret_request.environment
    )
    is_production = check_production_risk(secret_request.environment)
